...
public class CustomParticleEmitter
{
		...
		public float horizRatio = 1;
		public float vertRatio = 1;
		...
		public void dispose()
		{
			...
			xOffsetValue = null;
			yOffsetValue = null;
			spawnWidthValue = null;
			spawnHeightValue = null;
			spawnShapeValue = null;
			sprite = null;
			particles = null;
		}
		...
		public CustomParticleEmitter (BufferedReader reader) throws IOException {
			initialize();
			load(reader);
		}

		public CustomParticleEmitter (CustomParticleEmitter emitter) {
		...
		private void activateParticle (int index) {
			...
			particle.scaleX = scaleValue.newLowValue() / spriteWidth * horizRatio;
			particle.scaleDiffX = scaleValue.newHighValue() / spriteWidth * horizRatio;
			
			////////////////////////////////////////////////////////////////////
			particle.scaleY = scaleValue.newLowValue() / spriteHeight * vertRatio;
			particle.scaleDiffY = scaleValue.newHighValue() / spriteHeight * vertRatio;
			////////////////////////////////////////////////////////////////////
			
			if (!scaleValue.isRelative()) {
				particle.scaleDiffX -= particle.scaleX;
				particle.scaleDiffY -= particle.scaleY;
			}
			particle.setScale(particle.scaleX + particle.scaleDiffX * scaleValue.getScale(0) ,particle.scaleY + particle.scaleDiffY * scaleValue.getScale(0));
		...
		private boolean updateParticle (Particle particle, float delta, int deltaMillis) {
			...
			if ((updateFlags & UPDATE_SCALE) != 0)
				particle.setScale(particle.scaleX + particle.scaleDiffX * scaleValue.getScale(percent), particle.scaleY + particle.scaleDiffY * scaleValue.getScale(percent));
		...
		static class Particle extends Sprite {
			...
			float scaleX, scaleDiffX, scaleY, scaleDiffY;
